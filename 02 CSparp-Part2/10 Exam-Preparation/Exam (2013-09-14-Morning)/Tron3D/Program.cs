...
namespace Trails3D_Kenov
{
    class Program
    {
        // копирано от Trails 3D с поправки в convertCommands() - не работи, иска дебъгване :(, някой друг път
        ...
        static void Main()
        {
            ...
            string redCommandsInput = "MMLMMMMRMRMMLMMRMMRMLMMRMMRMLMMLMMMLMMM";
            //string blueCommandsInput = Console.ReadLine();
            string blueCommandsInput = "LMMMMRMRMMMLMMRMMMMLMLMMMMRMLMMRMMMMRMM";

            string[] dimentionsSplitted = dimentions.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            int X = int.Parse(dimentionsSplitted[1]);
            int Y = int.Parse(dimentionsSplitted[0]);
