...
namespace Infestation
{
    public class HoldingPen
    {
        ...
        {
            Console.Write(" = {0}: ", this.containedUnits.Count);
            foreach (var unit in this.containedUnits)
            {
                Console.Write("{0}, {1};", unit.Id, unit.IsDestroyed ? "1" : "0");
            }
            Console.WriteLine();
            ...
            Console.Write("   {0}: ", requestedInteractions.Count());
            foreach (var unit in requestedInteractions)
            {
                Console.Write("{0} => {1}; ", unit.SourceUnit.Id, unit.TargetUnit.Id);
            }
            Console.WriteLine();
            ...
            foreach (var interaction in requestedInteractions)
            {
                ...
                Console.WriteLine("\t{2} {3} => {0} {1}", interaction.TargetUnit.Id, interaction.TargetUnit.Health,
                    interaction.SourceUnit.Id, interaction.SourceUnit.Health);
            }            
