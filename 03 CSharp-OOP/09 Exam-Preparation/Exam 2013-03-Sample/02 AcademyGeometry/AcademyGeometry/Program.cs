...
public class Cylinder : Figure, IVolumeMeasurable, IAreaMeasurable
{
    ...
    public double Heigth { get { return (this.vertices[0] - this.vertices[1]).Magnitude; } }

    public Cylinder(Vector3D top, Vector3D bottom, double raduis)
        : base(top, bottom)
    {
        this.Raduis = raduis;
    }
    ...
    public double GetArea()
    {
        return 2 * Math.PI * this.Raduis * this.Raduis + (2*Math.PI * this.Raduis*this.Heigth);
    }

    public double GetVolume()
    {
        return Math.PI * this.Raduis * this.Raduis * this.Heigth;
    }
...
public class AdvancedFigureController : FigureController
{
    public override void ExecuteFigureCreationCommand(string[] splitFigString)
    {
        switch (splitFigString[0])
        {
            ...
            case "cylinder":
                {
                    Vector3D centerTop = Vector3D.Parse(splitFigString[1]);
                    Vector3D centerBottom = Vector3D.Parse(splitFigString[2]);
                    double radius = double.Parse(splitFigString[3]);
                    currentFigure = new Cylinder(centerTop, centerBottom, radius);
                    break;
                }  
        }
            ...
            case "volume":
                {
                    ...
                    {
                        Console.WriteLine("{0:0.00}", volumeFigure.GetVolume());
                    }
                    break;
                }
            case "normal":
                {
                    var flatFigure = this.currentFigure as IFlat;
                    if (flatFigure == null)
                    {
                        Console.WriteLine("undefined");
                    }
                    ...
                    {
                        Console.WriteLine("{0:0.00}", flatFigure.GetNormal());
                    }
                    break;
                } 
        }
